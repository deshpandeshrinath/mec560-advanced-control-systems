...
function [states] = comparisionAnalysis(time,states,time2,states2)
model_params;

figure;
subplot(2,1,1)
plot(time,states(:,1),'r-',time,states(:,2),'b-',time,states(:,3),'g-',time,states(:,1),'r.',time2,states2(:,2),'b.',time2,states2(:,3),'g.')
xlabel('time')
title('position and orientation')
legend('x_{tracked}','y_{tracked}','\theta_{tr}','x_d','y_d','\theta_d')
subplot(2,1,2)
plot(time,states(:,4),'r-',time,states(:,5),'b-',time,states(:,6),'g-',time2,states2(:,4),'r.',time2,states2(:,5),'b.',time2,states2(:,6),'g.')
xlabel('time')
title('velocities')
legend('V_x_{tracked}','V_y_{tracked}','\omega_{tracked}','V_{x_d}','V_{y_d}','\omega_d')
figure;
plot(time,states(:,9)*180/pi,'b-',time2,states2(:,7)*180/pi,'r-')
xlabel('time')
ylabel('steeringAnlges')
legend('\phi_Actual','\phi_d')
